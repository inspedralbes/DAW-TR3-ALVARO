<template>
  <div class="min-h-screen bg-background text-on-background font-body">
    <!-- Admin Header -->
    <header
      class="fixed top-0 left-0 right-0 z-50 h-16 bg-zinc-950/95 backdrop-blur-xl border-b border-white/5 flex items-center px-6 gap-4"
    >
      <div class="flex items-center gap-3">
        <div
          class="w-8 h-8 rounded-lg signature-pulse flex items-center justify-center"
        >
        ...
        <div>
          <span
            class="text-xs font-bold tracking-[0.25em] uppercase text-on-surface-variant"
            >Panell</span
          >
          <div
            class="text-sm font-black tracking-tighter bg-gradient-to-r from-fuchsia-400 to-cyan-400 bg-clip-text text-transparent uppercase font-headline leading-none"
          >
            TICKETMONSTER ADMIN
          </div>
        </div>
      ...
      <div class="flex items-center gap-3">
        <div
          class="hidden md:flex items-center gap-2 text-xs text-on-surface-variant bg-white/5 px-3 py-2 rounded-lg"
        >
