...
redis.on('message', (channel, message) => {
  ...
  try {
    ...
    if (rawData.event === 'seat_updated' || rawData.event === 'App\\Events\\SeatUpdated') {
      ...
      if (activeTimers.has(seatId)) {
        ...
        console.log(`Timer cancelado para asiento ${seatId} porque ha cambiado a status: ${status}`);
      }

      // Si el evento fue que se "Reservó"
      if (status === 'reserved') {
        const timeoutId = setTimeout(async () => {
          console.log(`[TIMEOUT 15s] Timer expirado para asiento ${seatId}. Verificando BBDD...`);
          try {
            // Revisamos en vivo localmente si sigue reservado
            const [rows] = await db.query('SELECT status FROM seats WHERE id = ?', [seatId]);
            if (rows.length > 0 && rows[0].status === 'reserved') {
                console.log(`Asiento ${seatId} seguía bloqueado. Forzando liberación (Timeout Inactividad).`);
...
app.get('/api/health', async (req, res) => {
  try {
    ...
    res.json({ status: 'ok', db: 'connected', message: 'Node.js y DB conectados' });
  } catch (error) {
...
server.listen(PORT, () => {
  console.log(`🚀 Servidor de Tiempo libre (Node.js) enlazado al puerto ${PORT}`);
});
